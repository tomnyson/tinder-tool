...
const StorageHelper = {
  ...
  async get(keys) {
    if (!this.isValid()) {
      console.warn('StorageHelper.get — extension invalid, using localStorage fallback');
  ...
  async set(obj) {
    if (!this.isValid()) {
      console.warn('StorageHelper.set — extension invalid, using localStorage fallback');
